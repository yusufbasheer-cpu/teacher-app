...
- Checkpoint 19: confirmed via `vercel env ls` that the new Vercel deployment (`teacher-app/layah-backend-python`) has **zero** environment variables configured — geo verified fully working with none. `TURNSTILE_SECRET_KEY` was deliberately not configured there (no safe/authorized way to obtain the real value this session), so verify-captcha's Turnstile provider path remains unverified remotely; only its missing-secret contract path was confirmed. `PYTHON_BACKEND_URL`/`BACKEND_ROUTE_GEO`/`BACKEND_ROUTE_VERIFY_CAPTCHA` remain unset on the Next side (`project-scquo`) — no routing enabled.
- Checkpoint 20 added `PYTHON_BACKEND_BYPASS_SECRET` (new app-level variable, `src/lib/backend-routing.ts`'s `applyDeploymentProtectionBypass`): attaches Vercel's own `x-vercel-protection-bypass` header to the outgoing proxy request, only when explicitly set — inert/no-op otherwise. Also temporarily set `TURNSTILE_SECRET_KEY` on the backend to Cloudflare's official public test values (not real secrets) to exercise the real provider path; removed afterward, restoring the zero-secret default. `PYTHON_BACKEND_URL`/`BACKEND_ROUTE_GEO`/`BACKEND_ROUTE_VERIFY_CAPTCHA`/`PYTHON_BACKEND_BYPASS_SECRET` were briefly set on `project-scquo`'s Preview scope for an abandoned Preview-deploy attempt and fully removed afterward — none persist anywhere. A Protection Bypass secret was briefly exposed in tool output during this checkpoint and immediately rotated with user authorization; see `REMOTE_ROUTING_VALIDATION.md` for the full incident record.
