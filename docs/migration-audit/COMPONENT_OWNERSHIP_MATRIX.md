...
| `src/lib/backend-routing.ts`, geo/verify-captcha routing seams | MIGRATION INFRASTRUCTURE | server-only per-endpoint routing and rollback control for Track B pilots | Allowlists `GET /api/geo` and (Checkpoint 18) `POST /api/auth/verify-captcha` by name in a fixed map; not an API gateway and not AI-service ownership. Both default to Next; neither is cut over. |
