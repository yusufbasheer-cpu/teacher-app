...
Use the framework above to silently shape objectives, terminology, grade-appropriateness, pedagogical structure, and relevant local/contextual examples. Do NOT insert explicit statements such as "this aligns with...", "this meets...", "according to [framework]...", or "this supports curriculum standards..." into ordinary sections — that reads as narration, not teaching content, and is not something this application formally verifies. Only name the framework or its bodies explicitly inside a section that is ALREADY part of the requested output structure and whose deliberate, pre-existing purpose is to report curriculum/standards connections (for example an existing "real-life and cross-curricular connection" slide). Do NOT invent, add, or append a new standalone section, heading, or paragraph purely to report framework alignment — if the requested output structure has no such section, let the framework work silently through the sections that already exist and say nothing explicit about it. Never claim the lesson "is designed to meet," "complies with," or "is aligned with" an official standards body's requirements as a blanket statement — this application does not perform formal standards certification.`;
}
