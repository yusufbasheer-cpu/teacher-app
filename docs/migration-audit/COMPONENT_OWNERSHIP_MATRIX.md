...
| `src/lib/backend-routing.ts`, geo routing seam | MIGRATION INFRASTRUCTURE | server-only per-endpoint routing and rollback control for Track B pilots | Currently allowlists only `GET /api/geo`; not an API gateway and not AI-service ownership. |
