...
| --- | --- | --- | --- | --- |
| `GET /api/geo` | `REMOTE_ROUTING_VALIDATED` (local Next dev + remote Preview backend; not yet a real Next Preview) | ready for explicit opt-in via `BACKEND_ROUTE_GEO=python`; routing mechanism proven against the real remote backend | not cut over | low-risk Track B pilot; Checkpoint 15 local, Checkpoint 19 direct-remote, Checkpoint 20 full routing+rollback proven via local Next dev → real Vercel Preview backend (an actual Next Preview deploy is blocked by an out-of-scope `project-scquo` setting) — see `REMOTE_ROUTING_VALIDATION.md`; `PYTHON_BACKEND_URL`/`BACKEND_ROUTE_GEO` still unset everywhere |
...
| `POST /api/auth/verify-captcha` | `REMOTE_ROUTING_AND_PROVIDER_VALIDATED` (local Next dev + remote Preview backend) | disabled-by-default opt-in via `BACKEND_ROUTE_VERIFY_CAPTCHA=python`; routing and real Turnstile provider path (both approve/reject) proven against the real remote backend | not cut over | Checkpoint 18 second Track B pilot; public, non-mutating, single external call (Cloudflare Turnstile); Python transport failure deliberately does NOT fall back to Next (single-use token risk); Checkpoint 20 exercised the full safe test matrix — including the real Turnstile provider branch, via Cloudflare's official public test credentials — through the actual Next routing path — see `REMOTE_ROUTING_VALIDATION.md`, `VERIFY_CAPTCHA_PYTHON_PARITY_CONTRACT.md` |
